#!/usr/bin/env python3
"""
Switch Atlas EZO ORP from UART mode to I2C mode.

Hardware context:
- ORP circuit is connected on P4 of an isolated carrier board.
- In UART mode, the status LED usually flashes.

This script sweeps common UART ports/baud rates and sends the ORP
I2C switch command for address 0x62 (decimal 98).
"""
...
TARGET_I2C_ADDR_DEC = 98   # 0x62 (Atlas ORP default)
TARGET_I2C_ADDR_HEX = "0x62"
...
def baud_sweep_switch_orp():
    ...
    print("=== Atlas ORP UART -> I2C Switch ===")
    ...
    print("\nSweep complete.")
    if any_response:
        print(f"Now check I2C for ORP at {TARGET_I2C_ADDR_HEX}:")
    else:
        print("No UART responses were seen. Still check I2C once, then verify P4 seating/UART config.")
