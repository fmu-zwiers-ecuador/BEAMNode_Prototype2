#!/usr/bin/env python3
"""
Switch Atlas EZO RTD from UART mode to I2C mode.

Hardware context:
- RTD circuit is connected to a UART-capable serial interface.
- In UART mode, the status LED usually flashes.

This script sweeps common UART ports/baud rates and sends the RTD
I2C switch command for address 0x66 (decimal 102).
"""
...
_serial = importlib.import_module("serial")

TARGET_I2C_ADDR_DEC = 102  # 0x66 (Atlas RTD default)
TARGET_I2C_ADDR_HEX = "0x66"
...
def baud_sweep_switch_rtd():
    ...
    print("=== Atlas RTD UART -> I2C Switch ===")
    ...
    for port in available_ports:
        ...
        for baud in baud_rates:
            try:
                ...
                ser = _serial.Serial(port, baud, timeout=0.8)
    ...
    print("\nSweep complete.")
    if any_response:
        print(f"Now check I2C for RTD at {TARGET_I2C_ADDR_HEX}:")
    else:
...
if __name__ == "__main__":
    try:
        baud_sweep_switch_rtd()
    except ModuleNotFoundError:
        print("Missing dependency: pyserial. Install with: pip3 install pyserial")
