# Author: Jackson Roberts | pH Logging Script
...
def parse_first_float(text: str, precision: int = 3):
    numbers = re.findall(r"[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?", text)
    if not numbers:
        raise Exception(f"Could not parse numeric pH value from response: {repr(text)}")
    return round(float(numbers[0]), precision)


def log_data():
    config = load_config()

    ph_config = config.get("atlas_ph", {})
    ...
    try:
        addr_hex = ph_config.get("address_hex") or "0x63"
        i2c_bus = ph_config.get("i2c_bus")
        bus_num = int(i2c_bus) if i2c_bus is not None else 1
        addr = int(str(addr_hex), 16)
    except Exception as e:
        print(
            f"Config Parse Error in atlas_ph: "
            f"address_hex={ph_config.get('address_hex')!r}, "
            f"i2c_bus={ph_config.get('i2c_bus')!r} | {e}",
            file=sys.stderr
        )
        sys.exit(1)
    ...
    try:
        with AtlasI2CDevice(addr, bus=bus_num) as dev:
            ...
            if status != 1 or "PH" not in info_norm:
                raise Exception(
                    f"Atlas pH identity check failed on bus {bus_num}, addr 0x{addr:02X}. "
            ...
            if status != 1:
                ...
                raise Exception(f"Atlas pH error code {status} ({desc}) | Raw: {raw}")

            if not text:
                raise Exception(f"Atlas pH returned empty payload | Raw: {raw}")

            value = parse_first_float(text, precision=3)
    ...
    sensor_dir = os.path.join(base_dir, ph_config.get("directory", "ph"))
    os.makedirs(sensor_dir, exist_ok=True)
    file_path = os.path.join(sensor_dir, ph_config.get("file_name", "ph_data.json"))
    ...
    try:
        ...
        print(f"Logged pH reading: {value}")
    except Exception as e:
