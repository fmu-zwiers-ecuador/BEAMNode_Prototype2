#!/usr/bin/env python3
"""
Switch Atlas EZO Dissolved Oxygen (DO) from UART mode to I2C mode.

Hardware context:
- DO circuit is connected to a UART-capable serial interface.
- In UART mode, the status LED usually flashes.

This script sweeps common UART ports/baud rates and sends the DO
I2C switch command for address 0x61 (decimal 97).
"""
...
TARGET_I2C_ADDR_DEC = 97   # 0x61 (Atlas DO default)
TARGET_I2C_ADDR_HEX = "0x61"
...
def baud_sweep_switch_do():
    ...
    print("=== Atlas DO UART -> I2C Switch ===")
    ...
    print("\nSweep complete.")
    if any_response:
        print(f"Now check I2C for DO at {TARGET_I2C_ADDR_HEX}:")
    else:
